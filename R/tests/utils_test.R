TestFuzzyFeatureNameExactMatch <- function() {
  ...
  res <- .FuzzyFeatureNameMatch(available.names, "x_1")
  checkEquals(res, "x_1")
}

TestFuzzyFeatureNameBroadMatch <- function() {
  available.names <- c("x_1", "y_2", "z_1")
  res <- .FuzzyFeatureNameMatch(available.names, "x.1")
  checkEquals(res, "x_1")

  res <- .FuzzyFeatureNameMatch(available.names, ".2")
  checkEquals(res, "y_2")
}

TestFuzzyFeatureNameNoMatch <- function() {
  available.names <- c("x_1", "y_2", "z_1")
  res <- .FuzzyFeatureNameMatch(available.names, "w_3")
  checkTrue(is.null(res))

  res <- .FuzzyFeatureNameMatch(available.names, "_1")
  checkTrue(is.null(res))
}
...
TestGetPrior <- function() {
  ...
  priors1 <- GetPrior.EMRE(mdl, "1__x.1")
  checkEquals(names(priors1), names(all.priors))

  priors2 <- GetPrior.EMRE(mdl, "1__x.1", start.iter = 11)
  checkEquals(names(priors2), names(all.priors)[2:4])

  priors3 <- GetPrior.EMRE(mdl, "1__x.1", start.iter = 11, end.iter = 39)
